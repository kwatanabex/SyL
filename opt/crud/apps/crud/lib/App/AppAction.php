...
abstract class AppAction extends SyL_ActionAbstract
{
    ...
    public function execute(SyL_ContextAbstract $context, SyL_Data $data)
    {
        ...
        try {
            ...
            $this->executeAction($context, $data);
     ...
     * @param SyL_Data データ管理オブジェクト
     */
    public abstract function executeAction(SyL_ContextAbstract $context, SyL_Data $data);
    ...
    private function displayHelp(SyL_Console $cmd, $file)
    {
        $help = <<<EOF
Usage:
  php {$file} -d <dir> -t <name> [-n <name>] [-f <name>]
  php {$file} -c <file> -t <name> [-n <name>] [-f <name>]
  php {$file} [option]
...
Options:
  ...
  -a <name>  CRUD name (default: table name)
  -f <file>  CRUD Config template file
...
EOF;
    ...
    protected function checkTableName($table_name)
    {
        if (!preg_match('/^[a-zA-Z0-9_]+$/', $table_name)) {
            throw new ErrorException("invalid table name (-t) or CRUD name (-n). [a-zA-Z0-9_] only ({$table_name})");
        }
    }
     ...
     */
    protected function checkTemplateFile($path)
    {
        if (!is_file($path)) {
            throw new ErrorException("CRUD Config template file (-f) not found ({$path})");
        }
    }
     ...
     * エンティティファイルを確認する
     *
     * @param string エンティティファイル
     */
    protected function getEntity($entity_file, $entity_classname)
    {
        if (!is_file($entity_file)) {
            throw new ErrorException("entity file not found ({$entity_file}). if dao is not created, create dao at dao.php");
        }
        include_once $entity_file;

        if (!class_exists($entity_classname)) {
            throw new ErrorException("entity class not found ({$entity_classname} in {$entity_file})");
        }
        return new $entity_classname();
    }
