 ...
 * @link       http://syl.jp/
 */
class SyL_FormElementSelectMultiple extends SyL_FormElementAbstract
{
    /**
     * フォーム要素の値
     *
    ...
    {
        // multiple select のため
        $this->setAttribute($this->prefix_id . $this->getNames(), 'id');
        $this->setAttribute($this->getNames(), 'name');

        $options = array();
        foreach ($this->options as $display_name => $value) {
            $selected = in_array((string)$value, $this->value, true) ? ' selected' : '';
            $options[] = sprintf('<option value="%s"%s>%s</option>', self::encode($value), $selected, self::encode($display_name));
        }

        return sprintf('<select %s multiple>%s</select>', $this->getAttributeString(), implode("\r\n", $options));
    }
    ...
    protected function getHtmlView()
    {
        ...
        foreach ($this->options as $display_name => $value) {
            if (in_array((string)$value, $this->value, true)) {
                $labels[] = self::encode($display_name);
            }
        }
        return $this->getHtmlSpan($this->getNames(), implode($this->separator, $labels)) . $this->getHtmlHidden();
    }
